const tick = (hands) => {
   ...
   * @see https://css-triks.com/updating-a-css-variable-with-javascript
   */
  // 초침: 1초당 6도(360deg/60s) 회전
  $secondHand.style.setProperty('--deg', seconds * 6);
  // 분침: 1시간당 360도, 1분당 6도 (360deg/60m), 1초당 0.1(60deg/60s) 회전
  // 10qns 59ch => 60 + 5.9 = 65.9deg / 11분 => 66 + 0 = 66deg
  ...
  $hourHand.style.setProperty('--deg', hours * 30 + minutes * seconds * (0.5 / 60));
};

const AnalogClock = ($container) => {
  const $template = document.createElement('template'); // 가상의 DOM
  $template.innerHTML = `
